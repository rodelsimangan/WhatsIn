...
namespace WhatsIn.Tests
{
    public abstract class WhatsInTestBase : AbpIntegratedTestBase<WhatsInTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected WhatsInTestBase()
        {
        ...
        protected void UsingDbContext(Action<WhatsInDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<WhatsInDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<WhatsInDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<WhatsInDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<WhatsInDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WhatsInDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<WhatsInDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WhatsInDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<WhatsInDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<WhatsInDbContext, Task<T>> func)
        {
            T result;
